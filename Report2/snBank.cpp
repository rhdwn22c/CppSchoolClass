...
class BankUser {
public:
    float specialYearInterest = 4.0f;
    float normalYearInterest = 3.0f;
};
...
int main() {
    ...
    if (whatSaving == '1') {
        noneTaxMoneyResult = bank.cal(bank, user.specialYearInterest);
    }
    else if (whatSaving == '2') {
        noneTaxMoneyResult = bank.cal(bank, user.normalYearInterest);
    }
